...
<?php
	$htmlclasses = "";
	if (argon_get_option('argon_page_layout') == "single"){
		$htmlclasses .= "single-column ";
	}
	if (argon_get_option('argon_page_layout') == "triple"){
		$htmlclasses .= "triple-column ";
	}
	if (argon_get_option('argon_page_layout') == "double-reverse"){
		$htmlclasses .= "double-column-reverse ";
	}
	if (argon_get_option('argon_enable_immersion_color') == "true"){
		$htmlclasses .= "immersion-color ";
	}
	if (argon_get_option('argon_enable_amoled_dark') == "true"){
		$htmlclasses .= "amoled-dark ";
	}
	if (argon_get_option('argon_card_shadow') == 'big'){
		$htmlclasses .= 'use-big-shadow ';
	}
	if (argon_get_option('argon_font') == 'serif'){
		$htmlclasses .= 'use-serif ';
	}
	if (argon_get_option('argon_disable_codeblock_style') == 'true'){
		$htmlclasses .= 'disable-codeblock-style ';
	}
	if (argon_get_option('argon_enable_headroom') == 'absolute'){
		$htmlclasses .= 'navbar-absolute ';
	}
	// [PHP 8.0+] 使用 match 表达式替代 if/else
	$banner_size = argon_get_option('argon_banner_size', 'full');
	$htmlclasses .= match($banner_size) {
		'mini'       => 'banner-mini ',
		'hide'       => 'no-banner ',
		'fullscreen' => 'banner-as-cover ',
		default      => '',
	};
	if (argon_get_option('argon_toolbar_blur', 'false') == 'true'){
...
?>
<?php
	$cardradius = argon_get_option('argon_card_radius');
	if ($cardradius == ""){
		$cardradius = "4";
	}
...
<head>
	...
	<?php if (argon_get_option('argon_enable_mobile_scale') != 'true'){ ?>
	...
	<?php
		...
		// [Argon Next] 默认使用本地自托管字体，debug 模式下使用 Google CDN
		if (argon_get_option('argon_disable_googlefont') != 'true') {
			if (function_exists('argon_next_is_debug_mode') && argon_next_is_debug_mode()) {
				wp_enqueue_style("googlefont", "//fonts.googleapis.com/css?family=Open+Sans:300,400,600,700|Noto+Serif+SC:300,600&display=swap");
			} else {
				wp_enqueue_style("googlefont", $GLOBALS['assets_path'] . "/assets/fonts/google-fonts-local.css", null, $GLOBALS['theme_version']);
			}
		}
		wp_enqueue_script("argon_js_merged", $GLOBALS['assets_path'] . "/assets/argon_js_merged.js", null, $GLOBALS['theme_version'], true);
		wp_enqueue_script("argonjs", $GLOBALS['assets_path'] . "/assets/js/argon.min.js", null, $GLOBALS['theme_version'], true);
	?>
	<?php wp_head(); ?>
	<?php $GLOBALS['wp_path'] = argon_get_option('argon_wp_path') ?: '/'; ?>
	<script>
		document.documentElement.classList.remove("no-js");
		var argonConfig = {
			wp_path: "<?php echo $GLOBALS['wp_path']; ?>",
			language: "<?php echo argon_get_locate(); ?>",
			dateFormat: "<?php echo argon_get_option('argon_dateformat', 'YMD'); ?>",
			<?php if (argon_get_option('argon_enable_zoomify') == 'true'){ ?>
				zoomify: {
					duration: <?php echo argon_get_option('argon_zoomify_duration', 200); ?>,
					easing: "<?php echo argon_get_option('argon_zoomify_easing', 'cubic-bezier(0.4,0,0,1)'); ?>",
					scale: <?php echo argon_get_option('argon_zoomify_scale', 0.9); ?>
				},
			<?php } else { ?>
				zoomify: false,
			<?php } ?>
			pangu: "<?php echo argon_get_option('argon_enable_pangu', 'false'); ?>",
			<?php if (argon_get_option('argon_enable_lazyload') != 'false'){ ?>
				lazyload: {
					threshold: <?php echo argon_get_option('argon_lazyload_threshold', 800); ?>,
					effect: "<?php echo argon_get_option('argon_lazyload_effect', 'fadeIn'); ?>"
				},
			<?php } else { ?>
				lazyload: false,
			<?php } ?>
			fold_long_comments: <?php echo argon_get_option('argon_fold_long_comments', 'false'); ?>,
			fold_long_shuoshuo: <?php echo argon_get_option('argon_fold_long_shuoshuo', 'false'); ?>,
			disable_pjax: <?php echo argon_get_option('argon_pjax_disabled', 'false'); ?>,
			pjax_animation_durtion: <?php echo (get_option("argon_disable_pjax_animation") == 'true' ? '0' : '600'); ?>,
			headroom: "<?php echo argon_get_option('argon_enable_headroom', 'false'); ?>",
			waterflow_columns: "<?php echo argon_get_option('argon_article_list_waterflow', '1'); ?>",
			code_highlight: {
				enable: <?php echo argon_get_option('argon_enable_code_highlight', 'false'); ?>,
				hide_linenumber: <?php echo argon_get_option('argon_code_highlight_hide_linenumber', 'false'); ?>,
				transparent_linenumber: <?php echo argon_get_option('argon_code_highlight_transparent_linenumber', 'false'); ?>,
				break_line: <?php echo argon_get_option('argon_code_highlight_break_line', 'false'); ?>
			}
		}
	</script>
	<script>
		...
		<nav id="navbar-main" class="navbar navbar-main navbar-expand-lg navbar-transparent navbar-light bg-primary headroom--not-bottom headroom--not-top headroom--pinned">
			<div class="container">
				...
				<div class="navbar-brand mr-0">
					<?php if (argon_get_option('argon_toolbar_icon') != '') { /*顶栏ICON(如果选项中开启)*/?>
						<a class="navbar-brand navbar-icon mr-lg-5" href="<?php echo argon_get_option('argon_toolbar_icon_link'); ?>">
							<img src="<?php echo argon_get_option('argon_toolbar_icon'); ?>">
						</a>
					<?php }?>
					<?php
						//顶栏标题
						$toolbar_title = argon_get_option('argon_toolbar_title') == '' ? get_bloginfo('name') : argon_get_option('argon_toolbar_title');
...
<section id="banner" class="banner section section-lg section-shaped">
	<div class="shape <?php echo argon_get_option('argon_banner_background_hide_shapes') == 'true' ? '' : 'shape-style-1' ?> <?php echo argon_get_option('argon_banner_background_color_type') == '' ? 'shape-primary' : argon_get_option('argon_banner_background_color_type'); ?>">
		<span></span>
		<span></span>
		<span></span>
		<span></span>
	...
	<?php
		$banner_title = argon_get_option('argon_banner_title') == '' ? get_bloginfo('name') : argon_get_option('argon_banner_title');
		$enable_banner_title_typing_effect = argon_get_option('argon_enable_banner_title_typing_effect') != 'true' ? "false" : argon_get_option('argon_enable_banner_title_typing_effect');
	?>
	<div id="banner_container" class="banner-container container text-center">
		<?php if ($enable_banner_title_typing_effect != "true"){?>
			...
			<?php echo argon_get_option('argon_banner_subtitle') == '' ? '' : '<span class="banner-subtitle d-block">' . argon_get_option('argon_banner_subtitle') . '</span>'; ?></div>
		<?php } else {?>
			<div class="banner-title text-white" data-interval="<?php echo argon_get_option('argon_banner_typing_effect_interval', 100); ?>"><span data-text="<?php echo $banner_title; ?>" class="banner-title-inner">&nbsp;</span>
			<?php echo argon_get_option('argon_banner_subtitle') == '' ? '' : '<span data-text="' . argon_get_option('argon_banner_subtitle') . '" class="banner-subtitle d-block">&nbsp;</span>'; ?></div>
		<?php }?>
	</div>
	<?php if (argon_get_option('argon_banner_background_url') != '') { ?>
		<style>
			section.banner{
...
<?php if (apply_filters('argon_page_background_url', argon_get_option('argon_page_background_url')) != '') { ?>
	<style>
		<?php if (argon_get_option('argon_page_background_banner_style', 'false') == 'transparent') { ?>
			#banner, #banner .shape {
		...
		#content:before {
			...
			bottom: 0;
			z-index: -2;
			background: url(<?php echo apply_filters('argon_page_background_url', argon_get_option('argon_page_background_url'));?>);
			background-position: center;
			background-size: cover;
			background-repeat: no-repeat;
			opacity: <?php echo (argon_get_option('argon_page_background_opacity') == '' ? '1' : argon_get_option('argon_page_background_opacity')); ?>;
			transition: opacity .5s ease;
		}
		html.darkmode #content:before{
			filter: brightness(0.65);
		}
		<?php if (apply_filters('argon_page_background_dark_url', argon_get_option('argon_page_background_dark_url')) != '') { ?>
			#content:after {
				content: '';
				display: block;
				position: fixed;
				...
				bottom: 0;
				z-index: -2;
				background: url(<?php echo apply_filters('argon_page_background_dark_url', argon_get_option('argon_page_background_dark_url'));?>);
				background-position: center;
				background-size: cover;
				background-repeat: no-repeat;
				opacity: 0;
...
<?php if (argon_get_option('argon_show_toolbar_mask') == 'true') { ?>
	<style>
		#banner:after {
			content: '';
			width: 100vw;
...
<div id="float_action_buttons" class="float-action-buttons fabtns-unloaded">
	...
	<button id="fabtn_go_to_comment" class="btn btn-icon btn-neutral fabtn shadow-sm d-none" type="button" <?php if (argon_get_option('argon_fab_show_gotocomment_button') != 'true') echo " style='display: none;'";?> aria-label="Comment" tooltip="<?php _e('评论', 'argon'); ?>">
		<span class="btn-inner--icon"><i class="fa fa-comment-o"></i></span>
	</button>
	<button id="fabtn_toggle_darkmode" class="btn btn-icon btn-neutral fabtn shadow-sm" type="button" <?php if (argon_get_option('argon_fab_show_darkmode_button') != 'true') echo " style='display: none;'";?> aria-label="Toggle Darkmode" tooltip-darkmode="<?php _e('夜间模式', 'argon'); ?>" tooltip-blackmode="<?php _e('暗黑模式', 'argon'); ?>" tooltip-lightmode="<?php _e('日间模式', 'argon'); ?>">
		<span class="btn-inner--icon"><i class="fa fa-moon-o"></i><i class='fa fa-lightbulb-o'></i></span>
	</button>
	<button id="fabtn_toggle_blog_settings_popup" class="btn btn-icon btn-neutral fabtn shadow-sm" type="button" <?php if (argon_get_option('argon_fab_show_settings_button') == 'false') echo " style='display: none;'";?> aria-label="Open Blog Settings Menu" tooltip="<?php _e('设置', 'argon'); ?>">
